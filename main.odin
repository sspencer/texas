...
main :: proc() {
    ...
    set_tetromino(&game.ghost_tetro, game.tetro.id)
...
draw_score :: proc(g: ^Game) {
    ...
    rl.DrawText(fmt.ctprintf("Level: %d", g.level), tx, ty, font_size, rl.WHITE)
    r := 6
    ...
    ty += font_size + 40
    rl.DrawText(fmt.ctprintf("Score: %d", g.game_score), tx, ty, font_size, rl.WHITE)
    ty += 80
    ...
    msg:cstring = ""
    font_size = 32
    y := BORDER_HEIGHT + BORDER_SIZE + GAME_HEIGHT * BLOCK_SIZE / 2 - font_size / 2
    ...
    if len(msg) > 0 {
        ...
        rl.DrawText(msg, offset + GAME_WIDTH * BLOCK_SIZE / 2 - w/2 , y, font_size, rl.WHITE)
    }
}
...
update :: proc(g: ^Game, dt: f64) {
    ...
    if g.game_over && g.game_over_time - dt < 0 && g.game_launch_time - dt < 0 && rl.IsKeyPressed(.SPACE) {
        // new game in 3.., 2.., 1..
        g.game_launch_time = dt + GAME_LAUNCH_TIME
    }

    if g.debug == false && g.game_paused == false && dt - g.last_tick >= g.delta_time {
        g.last_tick = dt
        if collide(g) || move_tetro(g, { 0, 1 }) == false {
            lines := merge_tetro(g)
...
next_tetro :: proc(g: ^Game) {
    ...
    set_tetromino(&g.ghost_tetro, g.tetro.id)
    random_tetromino(&g.next_tetro)
}
...
compute_score :: proc(level, lines: int) -> int {
    ...
    case 3: return level * 500
    case 4: return level * 800
    case: return 0
    }
}
...
draw_ghost :: proc(t: Tetromino) {
    ...
    for r in 0 ..< 4 {
        for c in 0 ..< 4 {
            if t.shape[c][r] != 0 {
                //draw_block(t.x + c, t.y + r, size, Tetro_Color[t.shape[c][r]])
                draw_block(t.x + c, t.y + r, size, rl.PINK)
            }
        }
    }
}
...
draw_board :: proc(g: ^Game) {
    ...
    for y in 0 ..< GAME_HEIGHT {
        for x in 0 ..< GAME_WIDTH {
            rec := rl.Rectangle{
                f32(x) * size, f32(y) * size, size, size
            }
            if g.board[y][x] == 0 {
                ...
                rl.DrawRectangleRounded(rec, 0.4, 32, Tetro_Color[g.board[y][x]])
            }
        }
    }
}
...
draw_game_over :: proc(g: ^Game, dt: f64) {
    ...
    if game_over_text {
        border :i32 = 10
        y :i32 = 50
        rec := rl.Rectangle{ f32(border), f32(y), GAME_WIDTH * f32(BLOCK_SIZE) - f32(border) * 2.0, 140 }
...
collide :: proc(g: ^Game) -> bool {
    for c in 0 ..< 4 {
        for r in 0 ..< 4 {
            ...
            if g.tetro.x + c < 0 || g.tetro.x + c >= GAME_WIDTH || g.tetro.y + r < 0 || g.tetro.y + r >= GAME_HEIGHT {
                return true
            }

            // Check if the tetromino collides with existing blocks on the board
            if g.board[g.tetro.y + r][g.tetro.x + c] != 0 {
                return true
            }
        }
    }
